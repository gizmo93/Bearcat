    ...
    : IHosterWithFolders
{
        ...
        foreach (var attempt in Enumerable.Range(1, UploadAttempts))
        {
            try
            {
                logger.LogInformation(
                    "Uploading file {FilePath} to Fast2Share (Attempt {Attempt})",
                ...
                {
                    progress.ReportBytesTransferred(fileInfo.Length);
                }
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(
                    ex,
                    "Error while uploading file {FilePath} to Fast2Share on attempt {Attempt}: {ErrorMessage}",
                    fileDto.FullFileName,
                    attempt,
                    message
                );
    ...
    {
        var config = JsonSerializer.Deserialize<Fast2ShareConfig>(serializedConfig);

        return config
            ?? throw new InvalidOperationException("Failed to deserialize Fast2Share config");
    }

    public string SerializeHosterConfig(Dictionary<string, string> hosterConfig)
    {
        var config = new Fast2ShareConfig
        {
            ApiKey =
                hosterConfig.GetValueOrDefault(nameof(Fast2ShareConfig.ApiKey)) ?? string.Empty,
        };
