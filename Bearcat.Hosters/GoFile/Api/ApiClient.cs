    ...
    ILogger<ApiClient> logger)
{
    ...
    public async Task<IReadOnlyDictionary<string, (bool IsOnline, string? ErrorMessage)>> CheckOnlineStatusAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var checkOnlineStatusTasks = fileUrls
            .Select(fileUrl => CheckFileOnlineStatusAsync(
                fileUrl: fileUrl,
                apiToken: apiToken,
                semaphore: semaphore,
                cancellationToken: cancellationToken))
    ...
    private async Task<(string FileUrl, bool IsOnline, string? ErrorMessage)> CheckFileOnlineStatusAsync(
        string fileUrl,
        string apiToken,
        ...
        CancellationToken cancellationToken)
    {
        foreach (var attempt in Enumerable.Range(1, 3))
        {
                ...
                when (exception.StatusCode == HttpStatusCode.Unauthorized)
            {
                ...
                if (notPremiumError)
                {
                    logger.LogInformation("GoFile API returned notPremium error. Assuming all files are online");
            ...
            {
                logger.LogInformation("Rate limited by GoFile API, waiting 5 seconds before retrying (Attempt {Attempt})", attempt);
