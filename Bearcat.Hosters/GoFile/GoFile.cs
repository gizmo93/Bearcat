...
public class GoFile(ApiClient apiClient, ILogger<GoFile> logger) : IHoster
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var attempt in Enumerable.Range(1, 3))
        {
            try
            {
                logger.LogInformation("Uploading file {FilePath} to GoFile (Attempt {Attempt})",
            ...
            {
                logger.LogError("Error while uploading file {FilePath} to GoFile on attempt {Attempt}: {ErrorMessage}",
                    fileDto.FullFileName,
                    attempt,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var errorMessages = result.Values
            .Select(v => v.ErrorMessage)
            .Where(e => e is not null)
            .OfType<string>()
            .ToList();

        var linkStatuses = result
            .ToDictionary(
                r => r.Key,
                r => r.Value.IsOnline);
